class AutoPlay {
  ...
  play = async (callback) => {
    ...
    for (this.progress; this.progress < this.autoplay.length; this.progress++)
    {
      ...
      switch (command[0]) {
        case 'o':
        case 'on':
          this.canvas.keyOn(parseInt(command[2]) - 1, parseInt(command[1]) - 1, undefined, true, this.spam);
          if (this.highlight)
          {
        ...
        case 'f':
        case 'off':
          this.canvas.keyOff(parseInt(command[2]) - 1, parseInt(command[1]) - 1, undefined, true, this.spam);
          if (this.highlight)
          {
  ...
  wait(ms) {
    ...
    console.log(`Delay ${ms} (${adjusted_ms}) ms`)
    if (adjusted_ms > 5)
    {
